...
;  (set-table-size)
  ...
  (when with-rules
    (rules-new-game)
    (if (find id *games-list* :test #'string= :key #'id)
	(setq *cortex* (make-game-profiles id))
...
;url: flop
;params: c1,c2,c3
(defun flop(c1 c2 c3  &key (with-rules t))
  (unless (null (history +game+))
...
;url: turn
;params: c1
(defun turn(c1 &key (with-rules t))
...
;url: river
;params: c1
(defun river(c1  &key (with-rules t))
...
(defun holecards(c1 c2 &optional c3 c4 &key (with-rules t))
  (if (and c3 c4)
      (progn 
