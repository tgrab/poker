...
(defvar *log-request* nil)

(defmacro awhen(war &rest body)
 `(let ((it ,war))
    (when it ,@body)))
...
(defvar *listener* (make-listener :documentroot "/opt/code/lisp/thx" 
...
(defun do-it (listener stream)
	 ...
	 (when *log-request* (format t "~A : ~A ~&" url (request-body request)) )
	 (cond
...
(defmacro def-url (url name)
 `(setf (gethash ,url (listener-handlers *listener*))
      #'(lambda (request)
         (labels ((get-parameter(name) (cadr (assoc name (request-body request) :test #'string=))  ))
         (request-send-headers request)
...
(setf (listener-documentroot *listener*) "./files/")


(def-url "/admin_lc"
	(print (load (compile-file (get-parameter "file")))))


(def-url "/admin_eval"
	(print (eval (read-from-string (get-parameter "sexp")))))