...
(defclass game ()
  ((id :initarg :id :initform "000" :accessor game-id)
   ...
   (date :initarg :date :initform "Today" :accessor game-date)
   ...
   (game-kind :initarg :game-kind :initform 0 :accessor game-kind)
...
(defclass game-log(game)
  ;TODO main-player, money-lists
  ())



(defmethod print-object((g game) str)
  (format str "-----------Game: ~A [~A/~A] on ~A at ~A-------------~&" 
	  (game-id g) (small-blind g) (big-blind g) (game-date g)(game-time g))
...
(defmethod print-object((a player) str)
  ...
  (awhen (money a) (format str ", money: ~A" it)))
