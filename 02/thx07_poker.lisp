...
(defun new-hand(&optional (number 10))
  ...
  (setq *still-playing* nil)
  (setq *hs* 1000)
  (setq *players-on-flop* 0)
...
(defun flop(&rest a)
    ...
    (setq *stake* 0)
    (setq *players-on-flop* (players-left))
    (setq *money* (make-list *table-size* :initial-element 0))
    (setq *active-player* (next-player -1))
    (when *still-playing* 
      (setq *hs* 
	    (/ (moja-ocena (1- *players-on-flop*) )
	       (1- *players-on-flop*))))
    (push 'flop *history*))
...
(defun moja-ocena(&optional (ilosc-przeciwnikow 1) (reka *hand*) (stol *table*))
  (if (= (length *table*) 3)
      (pojedynek reka stol ilosc-przeciwnikow 400 (pary-sklanskyego 
			      (wyjmij (tasuj (talia)) (append reka stol))))
      (pojedynek reka stol ilosc-przeciwnikow 400
		 (pary-do-trojki reka (subseq (reverse stol) 0 3))) ))
