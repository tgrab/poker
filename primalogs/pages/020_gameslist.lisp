(defun card-view(c)
  (fmt "<img src='/images~A/~A.gif'>" *ctxt* c))

(defun cards-view(lista)
  (fmt "~{~A~}" (mapcar #'card-view lista)))

(define-easy-handler (gameslist :uri (fmt "~A/gameslist" *ctxt*)) (tn year month day)
  (page "Gameslist"
    ...
    (when (and tn (eq *selector* 'tname))
      ...
      (dolist (l (sort 
		  ...
		  #'string<))
	(when l (push (read-log l *logyear* *logmonth* *logday* tn ) *log-games1*) )) )
    (when *logday*
