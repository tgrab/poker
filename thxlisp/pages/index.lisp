(define-easy-handler (index0 :uri "/primalogs/") ()
 (redirect "/primalogs"))

(defvar *monthnames* '(("01" . "january")("02" . "february")("03" . "march")("04" . "april")))
(defvar *selector* 'tname) ;day month

(define-easy-handler (index :uri "/primalogs") (year month day selector)
  (page "Primalogs"
    (when selector
      (cond
	...
	(dolist (tn (get-log-tables *logyear* *logmonth* *logday*))
	  (a (:href (fmt "/primalogs/gameslist?tn=~A" tn) :style "font-size:0.8em") tn )
    ...
    (dolist (m (sort (get-log-months *logyear*) #'string> ))
      ...
      (dolist (d (sort (get-log-days *logyear* m) #'string<))
		...
		(a (:href (fmt "/primalogs/gameslist?year=~A&month=~A&day=~A" *logyear* m d) :style "margin:2px") d))))
