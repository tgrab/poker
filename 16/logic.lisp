; binding to cons (?x wartosc)
; bindings to lista podstawien ((?x wart1) (?y wart2))
...
(defun show-db()
    (mapc #'(lambda (x) (print (get x 'clauses)) ) *db-predicates*))
...
;; -------- WNIOSKOWANIE ----------------------------------
(defparameter *computable* '(game-prop = < > <= >= ?- figura kolor progn 1+ or))
...
(defmacro set-clause( clause )
    `(progn
      (setf (get ',(first clause) 'clauses) nil)
      (<- ,clause)))
