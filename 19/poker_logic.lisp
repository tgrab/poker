...
;; -------- WNIOSKOWANIE ----------------------------------
(defparameter *computable* '(= < > <= >= nie lub))
...
(defmacro lub(&rest exprs)
    `(or ,@(mapcar #'(lambda (e) `(?- ,e)) exprs)))
...
(defmacro rozwin-regule(pred action)
  `(let ((val (?- ,pred)))
    (when val
      (format nil "~A ~A" ,action (subst-bindings val '?rule)))))

(defmacro rozwin-fuzzy-regule(pred action)
  `(let ((val (?- ,pred)))
    (when val
      (format nil "~A ~A ~A" ,action (subst-bindings val '?level) (subst-bindings val '?rule)))))


;;-----------------------------------------------------------------------------------
(defun tokens (str &optional (delimiters (list #\Space))  (start 0))
"Arg: str:String &optional (delimiters:Lista_znakow (list #\Space))  (start:Int 0)
Out: Lista_String"
