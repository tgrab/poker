...
public class Img {
    ...
    static void write(BufferedImage im,String nazwa) {
        try {
    ...
    void im2W() {
        //punkty prawie biale zamienia na biale
        for (int x=0;x<width;x++)
            for (int y=0;y<height;y++) {
            int kolor=image.getRGB(x,y);
            Color c=new Color(kolor);
    ...
    void adjustBW() {
        //punkty prawie biale zamienia na biale
        //punkty prawie czarne zamienia na czarne
        for (int x=0;x<width;x++)
            for (int y=0;y<height;y++) {
            int kolor=image.getRGB(x,y);
            Color c=new Color(kolor);
            if (c.getRed()>220 & c.getGreen()>220 & c.getBlue()>220)
                image.setRGB(x,y,Color.white.getRGB());
            if (c.getRed()<35 & c.getGreen()<35 & c.getBlue()<35)
                image.setRGB(x,y,Color.black.getRGB());
            }
    }
    
    void im2Gray() {
        ColorConvertOp filtr=new ColorConvertOp(ColorSpace.getInstance(ColorSpace.CS_GRAY),null);
        image=filtr.filter(image,null);
    }
    ...
    boolean includes(int startx,int starty, Img what) {
        BufferedImage b=what.image;
        for (int c=0;c<what.width;c++)
            for (int r=0;r<what.height;r++)
                if (image.getRGB(startx+c,starty+r)!=b.getRGB(c,r)) return false;
        return true;
    }
    
    Point includes(Rectangle where, Img what) {
        //BufferedImage b=what.image;
        int startx=where.x;
        int starty=where.y;
        for (int c=0;c<where.width-what.width;c++)
            for (int r=0;r<where.height-what.height;r++)
                if (includes(startx+c,starty+r,what))
                    return new Point(startx+c,starty+r);
        return null;
    }
    ...
    boolean isRectangle(int x0,int y0,int szer,int wys, int color) {
        ...
        if (image.getRGB(x0+szer,y0+wys)!=color) return false;
