...
(defun create-games-list-set(start-period end-period table-name &optional (casino "crypto"))
  (poker-update (format nil "insert into games_list values(DEFAULT,'~A','~A',~A,~A)" casino table-name start-period end-period))  )

(defun get-games-list(nr)
  (let ((zbior (poker-query (format nil "select * from games_list where nr=~A" nr))))
    (when zbior
      (let ((pocz (nth 3 (car zbior))) 
	    (kon (nth 4 (car zbior)))
	    (cas (nth 1 (car zbior)))
	    (tn (nth 2 (car zbior))))
	(poker-query (format nil "select id,czas from ~A where czas>=~A and czas<=~A and table_name='~A' order by czas" cas pocz kon tn))
    ))))